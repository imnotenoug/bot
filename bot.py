...
async def handle_status(request):
  ...
  key = username.lower()
  data = get_or_create_user(key)
  current_data = user_controls[key]

  # Return a copy so we can clear one-shot triggers safely
  response_data = current_data.copy()

  # Clear triggers after they are read once by Roblox
  if current_data.get("serverhop"):
    current_data["serverhop"] = False
  if current_data.get("fling"):
    current_data["fling"] = False
...
@bot.tree.command(
    name="script",
    description="Get the complete execution loadstring and script source.",
)
async def script_command(interaction: discord.Interaction):
    ...
    async with session.get("https://awh.filho.wtf/full.lua") as resp:
      if resp.status != 200:
        await interaction.followup.send(
            "Failed to retrieve script source from remote server.", ephemeral=True
        )
        return
  ...
  loadstring_code = (
      'loadstring(game:HttpGet("https://awh.filho.wtf/full.lua"))()'
  )
...
async def autoreset_command(
  ...
  user_controls[key][setting_key] = state.value == "enable"

  embed = discord.Embed(
      title="Parameters Updated",
      description=(
          f"Auto-reset for **{mode.name}** class set to"
          f" **{state.name.lower()}** on account **{username}**."
      ),
      color=0xEB459E,
  )
  await interaction.response.send_message(embed=embed, ephemeral=False)
